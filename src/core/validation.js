...
export function isTimeEntry(value) {
  return Boolean(value && isString(value.id) && isString(value.activity) &&
    (value.categoryId === null || isString(value.categoryId)) && isDate(value.date) &&
    isTime(value.start) && isTime(value.end) && isFiniteNumber(value.mins) && value.mins >= 0);
}
export function isSavedActivity(value) {
  return Boolean(value && isString(value.id) && isString(value.label) &&
