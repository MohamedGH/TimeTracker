...
export function renameCategory(categories, id, label) {
  ...
  return categories.map(category => category.id === id
...
export function moveCategory(categories, id, parentId = null) {
  ...
  if (parentId && wouldCreateCycle(categories, id, parentId)) {
...
export function deleteCategory(categories, id, { cascade = false } = {}) {
  const category = categories.find(item => item.id === id);
  if (!category) throw new Error('Catégorie introuvable.');
