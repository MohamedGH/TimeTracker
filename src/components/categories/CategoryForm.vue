...
const color = ref(props.category?.color ?? '#5C7A5E');
...
<template>
  <form class="category-form" @submit.prevent="submit">
    <label>Nom
      <input v-model="label" maxlength="120" autocomplete="off" placeholder="Ex. Travail, Sport..." required />
    </label>
    ...
    <label v-if="!category || !category.builtin">Parent
